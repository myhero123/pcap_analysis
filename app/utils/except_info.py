        ...
        if pcap.haslayer(TCP):
            ...
            if src == host_ip:
                ...
                if sport in WARN_DICT:
                    portwarn_list.append({'ip_port': ip+':'+str(sport), 'warn': WARN_DICT[sport], 'time': time.strftime(
                        '%Y-%m-%d %H:%M:%S', time.localtime(pcap.time)), 'data': pcap.summary()})
                elif dport in WARN_DICT:
                    portwarn_list.append({'ip_port': ip+':'+str(dport), 'warn': WARN_DICT[dport], 'time': time.strftime(
                        '%Y-%m-%d %H:%M:%S', time.localtime(pcap.time)), 'data': pcap.summary()})
                else:
                    pass
            elif dst == host_ip:
    ...
    for web in webdata:
        ...
        for pattn, attk in ATTACK_DICT.items():  # 特征码和攻击名称
            if pattn.upper() in data.upper():
                webwarn_list.append({'ip_port': web['ip_port'].split(':')[
                                    0]+':'+web['ip_port'].split(':')[1], 'warn': attk, 'time': pattn, 'data': data})
    ip_count = collections.Counter(webbur_list)
    warn_ip = {k: y for k, y in ip_count.items() if y > 10}
    for ip, count in warn_ip.items():
        webwarn_list.append(
            {'ip_port': ip, 'warn': u'HTTP暴力破解', 'time': str(count), 'data': None})
    return webwarn_list
...
def ftp_warning(PCAPS, host_ip):
    ...
    for ip, count in warn_ip.items():
        ftpwarn_list.append(
            {'ip_port': ip, 'warn': u'FTP暴力破解', 'time': str(count), 'data': None})
    return ftpwarn_list
...
def telnet_warning(PCAPS, host_ip):
    ...
    for ip, count in warn_ip.items():
        telnetwarn_list.append(
            {'ip_port': ip, 'warn': u'Telnet暴力破解', 'time': str(count), 'data': None})
...
def arp_warning(PCAPS):
    ...
    for src, summary in arpsrc_dict.items():
        if len(set(summary)) == 1:
            ...
            arpwarn_list.append({'ip_port': src, 'warn': u'ARP欺骗', 'time': set(
                [s.split()[-1] for s in summary]), 'data': None})
    return arpwarn_list
