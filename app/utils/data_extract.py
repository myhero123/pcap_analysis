...
def web_data(PCAPS, host_ip):
    ...
    for ip_port, load_list in ip_port_ids_dict.items():
        ...
        if ('gbk' in tmp_data) or ('GBK' in tmp_data):
            ...
            data = tmp_data
        ip_port_data_list.append({'data_id': data_id, 'ip_port': ip_port, 'data': data, 'raw_data': raw_data, 'lens': '%.3f' % (
...
def mail_data(PCAPS, host_ip):
    ...
    for pcap in PCAPS:
        if pcap.haslayer(TCP) and pcap.haslayer(Raw):
            ...
            if sport == 110:
                ...
                pass
        id += 1
    # {'192.134.13.234:232':[2,3,4,5],'192.134.13.234:232':[4,3,2,4,3]}
    ip_port_ids_dict = OrderedDict()
    for ip_port_id in ip_port_id_list:
        if ip_port_id['ip_port'] in ip_port_ids_dict:
            ip_port_ids_dict[ip_port_id['ip_port']].append(
    ...
    for ip_port, load_list in ip_port_ids_dict.items():
        ...
        ip_port_data_list.append({'data_id': data_id, 'ip_port': ip_port, 'data': data, 'raw_data': raw_data,
                                 'parse_data': parse_data, 'lens': '%.3f' % (sum([len(corrupt_bytes(PCAPS[i])) for i in load_list])/1024.0)})
    return ip_port_data_list
...
def smtp_parse(raw_data):
    ...
    username = base64.b64decode(base64padding(username_.group(1))).decode(
        'UTF-8') if username_ else None
    password = base64.b64decode(base64padding(password_.group(1))).decode(
        'UTF-8') if password_ else None
    ...
    if mailsubject_:
        ...
        if mailsubject_ and '=?' in mailsubject_:
            ...
            mailsubject = base64.b64decode(base64padding(
                mailsubject_[3])).decode(mailsubject_[1], 'ignore')
        else:
    ...
    if mailcontent_:
        ...
        mailcontent = base64.b64decode(base64padding(
            mailcontent_)).decode(charset, 'ignore')
    else:
        mailcontent = None
    attachs_dict = findmail_attachs(raw_data)
    parse_data = {'username': username, 'password': password, 'maildate': maildate, 'mailfrom': mailfrom, 'mailto': mailto,
                  'mailcc': mailcc, 'mailsubject': mailsubject, 'mailmessageid': mailmessageid, 'mailcontent': mailcontent, 'attachs_dict': attachs_dict}
    return parse_data
        ...
        if 'filename' in data:
            ...
            if filename_:
                ...
                if filename_ and '=?' in filename_:
                    ...
                    charset = filename_[1]
                    filename = base64.b64decode(base64padding(
                        filename_[3])).decode(charset, 'ignore')
                else:
                    filename = filename_
            else:
                filename = 'unknow'
...
def pop3_parse(raw_data):
    ...
    if mailfrom_ and '=?' in mailfrom_:
        ...
        mailfrom_name = base64.b64decode(base64padding(
            mailfrom_[3])).decode(mailfrom_[1], 'ignore')
    ...
    if mailto_ and '=?' in mailto_:
        ...
        mailto_name = base64.b64decode(base64padding(
            mailto_[3])).decode(mailto_[1], 'ignore')
...
def client_info(PCAPS):
    ...
    for sess, payload in allpayloads_dict.items():
        ...
        if ether_dst and ether_src and clients_str:
            clientinfo_list.append({'sess': sess, 'ether_dst': ether_dst, 'ether_src': ether_src,
                                   'ip_src': ip_src, 'ip_dst': ip_dst, 'clients': clients_str[:-1], 'time': times})
...
def sen_data(PCAPS, host_ip):
    ...
    # Telnet协议帐号密码
    # login:
    telnet_pattern1 = re.compile(r'6c6f67696e3a.*?0d|4c6f67696e3a.*?0d')
    # Password:
    ...
    for telnet in telnetdata:
        ...
        if restp.strip():
            sendata_list.append(
                {'ip_port': telnet['ip_port'], 'result': result, 'data': telnet['data']})
    ...
    for ftp in ftpdata:
        ...
        if restp.strip():
            sendata_list.append(
                {'ip_port': ftp['ip_port'], 'result': result, 'data': data})
    ...
    for mail in maildata:
        ...
        if 'SMTP' in ip_port:
            ...
            mailuser_pwd_p = re.compile(r'LOGIN(.*?)\r\n', re.S)
            username_pwd_ = mailuser_pwd_p.search(data)
            username_pwd = username_pwd_.group(
                1).strip() if username_pwd_ else None
            if username_pwd:
                username = username_pwd.split()[0]
                password = username_pwd.split()[-1][1:-1]
            else:
    ...
    web_patternp = re.compile(
        '(({passwd})=(.*?))&'.format(passwd=passwd), re.I)
    ...
    for web in webdata:
        ...
        if tomcat:
            result = list(
                set([base64.b64decode(t.strip().replace('%3d', '=')) for t in tomcat]))
        if restp.strip():
            sendata_list.append(
                {'ip_port': web['ip_port'], 'result': result, 'data': data})
    return sendata_list
