    ...
    for web in webdata:
        ...
        for raw_data, data in zip(raw_data_list, data_list):
            if start:
                file_name = type + '_' + \
                    web['ip_port'].split(
                        ':')[0] + '_' + web['ip_port'].split(':')[1] + '_' + filename
                with open(folder + file_name, 'wb') as f:
                    f.write(raw_data.strip())
                web_list.append({'ip_port': web['ip_port'].split(':')[0]+':'+web['ip_port'].split(':')[
                                1], 'filename': (folder+file_name), 'size': '%.2f' % (os.path.getsize(folder+file_name)/1024.0)})
                start = False
                switch = False
            if switch:
            ...
            if 'GET' in data and 'HTTP/1.1' in data:
                try:
                    ...
                    if re.match(r'^[A-Za-z0-9_]*?\.[A-Za-z0-9_]*?$', filename):
                        filename = re.match(
                            r'^[A-Za-z0-9_]*?\.[A-Za-z0-9_]*?$', filename).group()
                        switch = True
                        start = False
                        type = 'GET'
                except:
    ...
    for ftp_cmd in ftp_cmd_data:
        ...
        if "PASV" in cmd_data:  # PASV模式,通过Web浏览器访问模式
            ...
            for ftp in ftpdata[start:]:
                file_name = ftp['ip_port'].split(
                    ':')[0] + '_' + ftp['ip_port'].split(':')[1] + '_'+filename_list[count]
                with open(folder + file_name, 'wb') as f:
                    f.write(ftp['raw_data'])
                count += 1
                ftp_list.append({'ip_port': ftp['ip_port'].split(':')[0] + ':' + ftp['ip_port'].split(':')[
                                1], 'filename': folder+file_name, 'size': '%.2f' % (os.path.getsize(folder+file_name)/1024.0)})
            ...
            for port, filename in port_file_list:
                ...
                file_name = ftpdata['ip_port'].split(
                    ':')[0] + '_' + ftpdata['ip_port'].split(':')[1] + '_'+filename
                with open(folder + file_name, 'wb') as f:
                    f.write(ftpdata['raw_data'])
                ftp_list.append({'ip_port': ftpdata['ip_port'].split(':')[0] + ':' + ftpdata['ip_port'].split(':')[
                                1], 'filename': folder+file_name, 'size': '%.2f' % (os.path.getsize(folder+file_name)/1024.0)})
        else:
            pass
    return ftp_list
    ...
    for mail in maildata:
        ...
        for data in data_list:
            if switch:
                ...
                try:
                    ...
                    pass
                file_dict[mail['ip_port'].split(
                    ':')[0]+'_'+mail['ip_port'].split(':')[1]+'_'+filename] = filedata
                switch = False
            if 'filename' in data:
                switch = True
        ...
        for filename, filedata in file_dict.items():
            ...
            with open(folder+filename, mode, encoding=encoding) as f:
                f.write(filedata)
            mail_list.append({'ip_port': filename.split('_')[0]+':'+filename.split('_')[
                             1], 'filename': folder+filename, 'size': '%.2f' % (os.path.getsize(folder+filename)/1024.0)})
    return mail_list
